...
// posts __edit_mode_available / __edit_mode_set_keys / __edit_mode_dismissed) so the
// app can run standalone or inside an editing host. Ships a consistent set of controls
// (segmented radios, sliders, steppers, curated color chips, …).
...
export function useTweaks<T extends object>(defaults: T): [T, SetTweak<T>] {
  ...
  const setTweak = React.useCallback(((keyOrEdits: keyof T | Edits<T>, val?: unknown) => {
    const edits: Edits<T> = typeof keyOrEdits === "object" && keyOrEdits !== null
      ? keyOrEdits
      : ({ [keyOrEdits as keyof T]: val } as Edits<T>);
...
export function TweakRadio({ label, value, options, onChange }: {
  label: string; value: string; options: TweakOption[]; onChange: (v: string) => void;
}) {
...
// Relative-luminance contrast pick for checkmarks drawn over a swatch.
...
// TweakColor — curated color/palette picker.
